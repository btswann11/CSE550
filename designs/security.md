...
```mermaid
flowchart TD
    A[User navigates to website] --> B[Popup:<br/>• Do not share personal info<br/>• Verify translation accuracy<br/>• Are you 18 or older?]
    B --> C{User selects an option}
    C -- Yes --> D[Proceed to login screen]
    C -- No --> E[Show popup: "You must be 18+ to use this app." (requires a page refresh to try again)]
```
